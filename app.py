...
<body>
    <div class="header">
        ...
        <a href="/dashboard" class="back-btn">⬅️ Dashboard</a>
    </div>
    ...
    <div class="container">
        ...
        <div class="users-section">
            ...
            <input type="text" class="search-box" id="search" placeholder="🔍 Rechercher par nom, username...">
    ...
    <script>
        ...
        function displayUsers(users) {
            ...
            if (users.length === 0) {
                container.innerHTML = '<p style="text-align:center;padding:40px;color:#999">Aucun utilisateur</p>';
                return;
            }
            
            container.innerHTML = users.map(user => `
                <div class="user-card" onclick="viewUserDetails(${user.user_id})">
                    <div class="user-header">
                        <div>
                            <strong style="font-size:16px">@${user.username}</strong>
                            <span class="badge">${user.total_orders} commandes</span>
                        </div>
                    </div>
                    <div class="user-info">
                        <div><strong>Nom:</strong> ${user.first_name} ${user.last_name}</div>
                        <div><strong>ID:</strong> ${user.user_id}</div>
                        <div><strong>Première visite:</strong> ${new Date(user.first_seen).toLocaleDateString('fr-FR')}</div>
                        <div><strong>Dernière activité:</strong> ${new Date(user.last_activity).toLocaleDateString('fr-FR')}</div>
                    </div>
                </div>
            `).join('');
        }
        ...
        async function viewUserDetails(userId) {
            ...
            alert(`Commandes de l'utilisateur #${userId}:\n\n${data.orders.map(o => 
                `#${o.id} - ${o.service} - ${o.price}€ - ${o.status}`
            ).join('\n')}`);
        }
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=port): 1400px;
            margin: 20px auto;
            padding: 0 20px;
        }
        .stats-grid {
...
<body>
    ...
    <div class="container">
        <div class="card">
            <h2>Générer des commandes de test</h2>
            <form id="simulate-form">
                ...
                <button type="submit">Générer les commandes</button>
                <button type="button" class="back-btn" onclick="location.href='/dashboard'">Retour au dashboard</button>
            </form>
            <div id="result"></div>
        </div>
    </div>
    ...
    <script>
        document.getElementById('simulate-form').addEventListener('submit', async (e) => {
            ...
            const response = await fetch('/api/simulate', {
                ...
                body: JSON.stringify({ count, service, status })
            });
            ...
            if (data.success) {
                document.getElementById('result').innerHTML = `
                    <strong>✅ ${data.created} commandes créées avec succès !</strong>
                `;
            } else {
                document.getElementById('result').innerHTML = `
                    <strong style="color:#842029">❌ Erreur: ${data.error}</strong>
                `;
            }
        });
    </script>
